				...
				<form class="form-horizontal" action="{{ route('admin.product.store') }}" method="post" enctype="multipart/form-data">
					...
					<div class="form-group row" id="img">
						...
						<div class="col-md-5">
							...
							<img class="image_preview position-absolute img_prv" src="" style="right: 17px;">
						</div>
					</div>
					...
					<div class="form-group row">
						...
						<div class="col-md-5">
							<select name="type" id="type" class="form-control" required>
								<option value="1">{{ __('backend/default.general') }}</option>
								<option value="2">{{ __('backend/default.feature') }}</option>
							</select>
							<button type="submit" id="submit_" class="btn btn-primary float-right mt-3">{{ __('backend/default.submit') }}</button>
						</div>
					</div>
				</form>
			</div>
